...
должна сообщать ему об ошибке и снова запрашивать знак операции. Также сообщать
...
while True:
    try:
        number1, operation, number2 = [
                i for i in
                input(
                    'Введите через пробел математическое выражение (например 3 + 2):\n'
                    ).split()
                ]
    except ValueError:
    ...
    if operation == '0':
        print('Ошибка. Неправильное математическое выражение. Работа завершена')
        break
    elif operation == '+':
